...
author: 
  - name          : "Nicholas Fox"
    ...
    address       : "53 Avenue E, Room 429, Piscataway New Jersey 08854"
...
affiliation:
  - id            : "1"
    institution   : "Rutgers University, New Brunswick New Jersey, USA"
...
#creates figure
figure1 <- estimates %>% 
  ggplot(aes(x = estimate_type, y = mean)) + geom_point(size = 2, aes(color = estimate_type)) + coord_flip() + geom_errorbar(aes(ymin = lower, ymax = upper, width = 0, color = estimate_type)) + xlab("") + ylab("\nEstimated Prevalence of QRP Use (%)") + ylim(-22,60) + theme_classic() + theme(legend.position = "none") + geom_hline(yintercept=0, linetype = "dotted") + scale_x_discrete(labels = c("UCT Estimate", "Direct Estimate", "GNSUM Estimate")) + scale_color_manual(values=c("black", "black", "black"))
...
Additionally, QRP use is temporally isolated to "in the past 12 months".  Although some have found instances of underreporting when using a 12 month recall [@Landen1995; @Connelly1995], this time frame is used frequently to measure current behavior in major national data collection surveys such as the National Health Interview Survey (NHIS) [@NHIS2006] and the National Survey on Drug Use and Health [@Ahrnsbrak2017].
